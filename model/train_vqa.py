...
from models.model_vqa import ChangeDetector
...
parser.add_argument('--checkpoint', type=str, default='checkpoints/pretrained_models/checkpoint_152000.pt')
# parser.add_argument('--checkpoint', type=str, default='checkpoints/checkpoint_3000.pt')
        ...
        if t % cfg.train.snapshot_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_i, val_batch in enumerate(tqdm(val_loader)):
                    d_feats, sc_feats, labels, sc_pos_labels, masks, pair_index, d_adj_matrix, q_adj_matrix, question, seq_neg, keywords_sent = val_batch  # two stage
                    ...
                    gen_sents = change_detector.generator(d_feats, sc_feats, d_adj_matrix, q_adj_matrix, question,
                                                          sample=False, num_beams=3, top_p=0.77, max_length=90, min_length=1)

                    for val_j in range(len(gen_sents)):
                        gt = labels[val_j]
                        predict_sent = gen_sents[val_j]
                ...
                for metric, score in coco_eval.eval.items():
                    ...
                    eval_score[metric] = '%.3f' % score
                    # output.append(score)
                with open(os.path.join(output_dir, 'eval_scores.json'), 'a', newline='\n') as f:
                    data = json.dumps(eval_score, indent=1)
                    f.write(data)

                print('evaluated')

                if coco_eval.eval['CIDEr'] > best:
                    best = coco_eval.eval['CIDEr']
