...
class ChangeDetector(nn.Module):
    ...
    def forward(self, input_1, input_2, d_adj_matrix, q_adj_matrix, question, answer, seq_neg, keywords_sent):
        ...
        local_loss = gloria.local_loss(question_embed.last_hidden_state[:, 1:, :], keywords_embeds, keywords_len)
        global_loss = gloria.global_loss(image_feat, torch.mean(keywords_embeds, dim=1))
        ...
        decoder_output = self.text_decoder(decoder_input_ids, attention_mask=answer_token.attention_mask,
                                           encoder_hidden_states=question_embed.last_hidden_state, labels=decoder_targets,
                                           return_dict=True)

        loss_lm = decoder_output.loss

        return loss_lm + loss_cl + loss_itm

    def generator(self, input_1, input_2, d_adj_matrix, q_adj_matrix, question,
                  sample=False, num_beams=3, top_p=0.77, max_length=90, min_length=1):
        ...
        # text_embed = self.text_encoder(encoder_input_ids.clone(), attention_mask=question_token.attention_mask,return_dict=True, mode='text')
        # question_choice = self.question_head(text_embed.last_hidden_state[:,0,:])
        # question_cls = torch.argmax(question_choice, dim=-1).bool()
        # embeds = torch.stack((text_embed_bef.last_hidden_state[:, 0, :], text_embed_diff.last_hidden_state[:, 0, :]), dim=-1)
        # question_embed = torch.matmul(embeds, question_choice.unsqueeze(-1)).permute(0, 2, 1)

        question_att = torch.ones(question_embed.last_hidden_state.size()[:-1], dtype=torch.long).to(input_1.device)
        model_kwargs = {"encoder_hidden_states": question_embed.last_hidden_state, "encoder_attention_mask": question_att}

        input = [''] * batch_size
        input_ids = self.tokenizer(input, return_tensors='pt').input_ids.to(input_1.device)
        ...
        if sample:
            ...
            outputs = self.text_decoder.generate(input_ids=input_ids,
                                                 ...
                                                 temperature=1,
                                                 **model_kwargs)
            # outputs_diff = self.text_diff_decoder.generate(input_ids=input_ids,
            #                                      max_length=max_length,
            #                                      min_length=min_length,
            #                                      do_sample=True,
            #                                      top_p=top_p,
            ...
            #                                      **model_kwargs)
        captions = []
        # for i, (output, output_diff) in enumerate(zip(outputs, outputs_diff)):
        #     if question_cls[i]:
        #         caption = self.tokenizer.decode(output, skip_special_tokens=True)
        #     else:
        #         caption = self.tokenizer.decode(output_diff, skip_special_tokens=True)
