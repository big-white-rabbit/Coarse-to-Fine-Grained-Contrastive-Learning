...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # pt = torch.exp(-lm_loss)
            # loss_focal = (1 - pt)**gamma*lm_loss
            # lm_loss = torch.mean(loss_focal*alpha)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        if reduction=='none':
            masked_lm_loss = torch.sum(masked_lm_loss.view(labels.size(0), -1), dim=-1)
