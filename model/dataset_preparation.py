...
def save_h5(questions, answers, pos, label_start_idx, label_end_idx, feature_idx,max_seq = 60, times=0, length = 100):
    filename = os.path.join('data/replaced_dataset/VQA_mimic_dataset.h5')
    if times == 0:
...
def save_coco_format():
    ...
    path = 'data/replaced_dataset/mimic_pair_questions_replaced.csv'
    df = pd.read_csv(path)
    anno_list= []
    image_list = []
    for name in ['train','val','test']:
        ...
        json.dump(dict, open('data/replaced_dataset/mimic_gt_captions_%s.json'%name, 'w'))
        image_list = []
        anno_list = []
        print('saved')
def transform_questions2dataset(simple = False, custom_dataset = False):
    ...
    question_path = '.data/replaced_dataset/mimic_pair_questions_replaced.csv'
...
def main():
    ...
    if args.transform_dataset:
        transform_questions2dataset(simple=False)
